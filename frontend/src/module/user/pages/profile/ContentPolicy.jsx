...
export default function ContentPolicy() {
    ...
    const [data, setData] = useState({
        ...
        content: '<p>Loading content policy...</p>'
    })
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            // Fallback to Terms if specific content policy doesn't exist on backend
            const response = await api.get(API_ENDPOINTS.ADMIN.TERMS_PUBLIC)
            if (response.data.success) {
                setData({
                    ...response.data.data,
                    title: 'Content Policy' // Set specific title
                })
            }
        } catch (error) {
            ...
            setData({
                title: 'Content Policy',
                content: '<p>Unable to load content policy at the moment. Please try again later.</p>'
            })
        } finally {
            setLoading(false)
        }
        ...
        <AnimatedPage className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-[#0a0a0a] dark:to-[#1a1a1a]">
            <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-8">
                ...
                <motion.div
                    ...
                    className="bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-lg border-0 dark:border-gray-800 p-6 md:p-8 lg:p-10"
                >
