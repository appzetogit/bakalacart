    ...
    const filteredSections = (menu.sections || [])
      ...
      .map(section => {
        ...
        const availableItems = (section.items || []).filter(item => {
          ...
          // Ignored approval status check to restore full menu visibility
          const shouldShow = isAvailable;

          // Debug logging for filtered items
          if (!shouldShow) {
            console.log(`[USER MENU] Filtering out item "${item.name}": isAvailable=${item.isAvailable}`);
          }
        ...
        const availableSubsections = (section.subsections || [])
          .map(subsection => {
            const availableSubsectionItems = (subsection.items || []).filter(item => {
              ...
              // Debug logging for filtered items
              if (!shouldShow) {
                console.log(`[USER MENU] Filtering out subsection item "${item.name}": isAvailable=${item.isAvailable}`);
              }
