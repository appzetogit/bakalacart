...
export const sendOTP = asyncHandler(async (req, res) => {
  ...
  try {
    const result = await otpService.generateAndSendOTP(phone || null, purpose, email || null, 'restaurant');
    ...
    if (req.body.fcmToken && (req.body.platform === 'web' || req.body.platform === 'mobile')) {
      ...
      if (platform === 'web') {
        ...
        if (!restaurant.fcmTokens.includes(fcmToken)) {
          ...
          if (restaurant.fcmTokens.length > 10) restaurant.fcmTokens = restaurant.fcmTokens.slice(-10);
        }
      } else {
        ...
        if (!restaurant.fcmTokenMobile.includes(fcmToken)) {
          ...
          if (restaurant.fcmTokenMobile.length > 10) restaurant.fcmTokenMobile = restaurant.fcmTokenMobile.slice(-10);
        }
      }
      await restaurant.save();
    }
...
export const refreshToken = asyncHandler(async (req, res) => {
  ...
  logger.info('🔍 [Restaurant Refresh Token] Attempting to refresh', {
  ...
  try {
    ...
    try {
      ...
      if (decodedInfo) {
        logger.info('🔍 [Restaurant Refresh Token] Token decoded (without verification)', {
      ...
      logger.warn('⚠️ [Restaurant Refresh Token] Failed to decode token (diagnostic)', {
    ...
    logger.error('❌ [Restaurant Refresh Token] Error refreshing token', errorDetails);
...
export const saveFcmToken = asyncHandler(async (req, res) => {
  ...
  const userId = req.restaurant._id; // Corrected to use req.restaurant (from restaurant middleware)
  ...
  if (platform === 'web') {
    if (!restaurant.fcmTokens) restaurant.fcmTokens = [];
    if (!restaurant.fcmTokens.includes(token)) {
      restaurant.fcmTokens.push(token);
      if (restaurant.fcmTokens.length > 10) restaurant.fcmTokens = restaurant.fcmTokens.slice(-10);
    }
  } else {
    if (!restaurant.fcmTokenMobile) restaurant.fcmTokenMobile = [];
    if (!restaurant.fcmTokenMobile.includes(token)) {
      restaurant.fcmTokenMobile.push(token);
      if (restaurant.fcmTokenMobile.length > 10) restaurant.fcmTokenMobile = restaurant.fcmTokenMobile.slice(-10);
    }
  }

  await restaurant.save();
  console.log(`✅ [Backend] FCM token saved for restaurant ${userId} on platform ${platform}`);
...
export const removeFcmToken = asyncHandler(async (req, res) => {
  ...
  if (platform === 'web' && restaurant.fcmTokens) {
    restaurant.fcmTokens = restaurant.fcmTokens.filter(t => t !== token);
  } else if (platform === 'mobile' && restaurant.fcmTokenMobile) {
    restaurant.fcmTokenMobile = restaurant.fcmTokenMobile.filter(t => t !== token);
  }
