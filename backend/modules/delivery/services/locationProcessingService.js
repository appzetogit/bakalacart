...
import { getFirebaseDb } from '../../../shared/services/firebaseAdmin.js';
...
export async function generateRoutePolyline(start, waypoint, end) {
  try {
    ...
    const roundCoordForFirebase = (coord) => Math.round(coord * 1000) / 1000; // 3 decimal places for key
    const firebaseCacheKey = `${origin}_${destination}`.replace(/[.,|:]/g, '_');

    if (db) {
      try {
        const snapshot = await db.ref(`route_cache/${firebaseCacheKey}`).once('value');
        if (snapshot.exists()) {
          const cachedData = snapshot.val();
          // Check expiry
          if (!cachedData.expires_at || cachedData.expires_at > Date.now()) {
            console.log('✅ Using Firebase cached route polyline');
            return cachedData;
          }
        }
      } catch (err) {
        console.warn('⚠️ Error reading Firebase route_cache:', err.message);
      }
    }

    // Check local Memory cache (fallback/speed)
    const cached = directionsCache.get(cacheKey);
    if (cached && (Date.now() - cached.timestamp) < DIRECTIONS_CACHE_TTL_MS) {
      console.log('✅ Using local cached route polyline');
      return cached.route;
    }
    ...
    if (response.data?.routes?.[0]) {
      ...
      if (db) {
        db.ref(`route_cache/${firebaseCacheKey}`).set({
          ...routeData,
          cached_at: Date.now(),
          expires_at: Date.now() + 604800000 // Cache for 7 days in Firebase
        }).catch(err => console.error('❌ Error saving to Firebase route_cache:', err));
      }
...
export async function processLocationUpdate(riderId, orderId, rawLocation, routeInfo) {
  try {
    ...
    const db = getFirebaseDb();
    if (db) {
      // 1. Update active order tracking
      db.ref(`active_orders/${orderId}`).update({
        boy_id: riderId,
        boy_lat: result.lat,
        boy_lng: result.lng,
        last_updated: Date.now(),
        status: progress ? 'on_the_way' : 'active',
        polyline: progress?.polyline || (routePolylines.get(orderId)?.polyline) || null,
        distance: result.remainingDistance / 1000, // km
        duration: (result.remainingDistance / result.speed) / 60 // minutes (pseudo)
      }).catch(err => console.warn('⚠️ Firebase active_orders update failed:', err.message));

      // 2. Update global delivery boy status
      db.ref(`delivery_boys/${riderId}`).update({
        lat: result.lat,
        lng: result.lng,
        last_updated: Date.now(),
        status: 'online'
      }).catch(err => console.warn('⚠️ Firebase delivery_boys update failed:', err.message));
    }
